...
$sql_nfse = "SELECT * FROM nfse WHERE Numero = " . $numero . " AND Id != " . $_POST["id_nota_fiscal"] . " AND Codigo = " . $_SESSION['codigo'] . " ";
...
if ($notaExistente) {
    ...
    $sql .= "Tomador_Endereco_Numero = '" . $_POST["tomador_numero"] . "', ";
    ...
    $sql .= " WHERE Id = '" . $_POST["id_nota_fiscal"] . "'";
    //$sql .= " WHERE Id = '" . $_POST["id_nota_fiscal"] . "' AND Codigo = '" . $_SESSION['codigo'] . "'";

    // Armazenando a string SQL na variável de sessão
    $_SESSION['sql_string'] = $sql;

    if (execSQL($con, $sql)) {
        echo json_encode(['saved' => true, 'sql_string' => $_SESSION['sql_string'], 'id_nota_fiscal' => $_POST["id_nota_fiscal"]]);
    } else {
        echo json_encode(['saved' => false, 'error' => mysqli_error($con)]);
    }
}
